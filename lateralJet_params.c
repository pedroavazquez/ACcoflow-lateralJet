...
event init ( t = 0 )
{
	...
	if (!RESTORE) {
		...
		foreach() {
			...
			//f[] = ( (y < 0.5*params.R1) && (x < 0.5*LX) ? 1.0 : 0);
		}
	} else {
...
event end ( t = params.TEND ) {
	...
	// Update DUMP_FILE and save parameters (rank 0 only)
...
event dumps( i++ )
{
	...
	// Update DUMP_FILE to the latest dump and save parameters (rank 0 only)
	strncpy(params.DUMP_FILE, nameDump, sizeof(params.DUMP_FILE) - 1);
	params.DUMP_FILE[sizeof(params.DUMP_FILE) - 1] = '\0';
	if (pid() == 0)
		save_params("params.txt", &params);
